...
import { settimanaTipoDefinitiva } from './settimanaTipoDefinitiva.ts';
...
async function creaStagioneTest(pool: Pool): Promise<string> {
  const nome = `stagione-stt-frontend-test-${randomUUID()}`;
...
async function creaSlotTest(
...
): Promise<{ slotId: string; spazioId: string; impiantoDenominazione: string; spazioDenominazione: string }> {
  const impiantoDenominazione = `Impianto Stt Frontend Test ${randomUUID().slice(0, 8)}`;
  const impiantoRes = await pool.query<{ id: string }>(`INSERT INTO impianti (denominazione) VALUES ($1) RETURNING id`, [
    impiantoDenominazione,
  ]);
  const impiantoId = impiantoRes.rows[0]!.id;
  impiantiCreate.push(impiantoId);

  const spazioDenominazione = `Spazio Stt Frontend Test ${randomUUID().slice(0, 8)}`;
  ...
  return { slotId: slotRes.rows[0]!.id, spazioId, impiantoDenominazione, spazioDenominazione };
}

descrivi('settimanaTipoDefinitiva.ts', () => {
  ...
  afterAll(async () => {
    ...
    if (associazioniCreate.length > 0) {
      ...
      await pool.query('DELETE FROM convenzioni WHERE assegnazione_id IN (SELECT id FROM assegnazioni WHERE associazione_id = ANY($1::uuid[]))', [associazioniCreate]);
  ...
  async function creaDisciplinaTest(): Promise<string> {
    ...
    await pool.query(`INSERT INTO discipline_sportive (codice, denominazione) VALUES ($1, 'Disciplina Test Stt Frontend')`, [
      disciplinaCodice,
    ]);
  ...
  async function creaAssociazioneConDomandaAmmessa(
    ...
    const associazione = await creaAssociazione({
      denominazione: `ASD Stt Frontend ${label} ${suffisso}`,
      ...
      email: `asd-stt-frontend-${label}-${suffisso}@example.com`.toLowerCase(),
  ...
  it('settimanaTipoDefinitiva restituisce fasce con denominazioni/dettagli leggibili, efficacia e slot liberi', async () => {
    ...
    const slotLibero = await creaSlotTest(pool, stagioneId, impiantiCreate, 2);

    const associazioneId = await creaAssociazioneConDomandaAmmessa(persona, stagioneId, disciplinaCodice, 'S', slot.slotId);
    ...
    const assegnazioneRes = await pool.query<{ id: string }>(
      `INSERT INTO assegnazioni (slot_id, domanda_id, associazione_id, tipo, stato, valore_minuti)
       VALUES ($1, $2, $3, 'singola', 'validata', 60.000) RETURNING id`,
      [slot.slotId, domandaId, associazioneId],
    );
    const impiantoIstituzione = await pool.query<{ id: string }>(`INSERT INTO istituzioni_scolastiche (denominazione) VALUES ($1) RETURNING id`, [
      `Istituto stt frontend ${randomUUID()}`,
    ]);
    await pool.query(`UPDATE impianti SET istituzione_scolastica_id = $1 WHERE id = (SELECT impianto_id FROM spazi_sportivi WHERE id = $2)`, [
      impiantoIstituzione.rows[0]!.id,
      slot.spazioId,
    ]);
    await pool.query(
      `INSERT INTO convenzioni (assegnazione_id, istituzione_scolastica_id, stato, confermata_il, confermata_da_persona_fisica_id)
       VALUES ($1, $2, 'perfezionata', now(), $3)`,
      [assegnazioneRes.rows[0]!.id, impiantoIstituzione.rows[0]!.id, persona.persona.id],
    );

    await pool.query(`UPDATE stagioni_sportive SET stato = 'definitiva' WHERE id = $1`, [stagioneId]);

    const esito = await settimanaTipoDefinitiva(stagioneId);
    expect(Array.isArray(esito.fasce)).toBe(true);
    const fascia = esito.fasce.find((f) => f.slotId === slot.slotId);
    expect(fascia).toBeTruthy();
    expect(fascia!.associazioneId).toBe(associazioneId);
    expect(fascia!.associazioneDenominazione).toContain('ASD Stt Frontend S');
    expect(fascia!.tipo).toBe('singola');
    expect(fascia!.valoreMinutiAssegnato).toMatch(/^60(\.000)?$/);
    expect(fascia!.impiantoDenominazione).toBe(slot.impiantoDenominazione);
    expect(fascia!.spazioDenominazione).toBe(slot.spazioDenominazione);
    expect(fascia!.giornoSettimana).toBe(1);
    expect(fascia!.orarioInizio).toBe('09:00');
    expect(fascia!.orarioFine).toBe('10:00');
    expect(fascia!.durataMinuti).toBe(60);
    expect(fascia!.efficace).toBe(true);

    expect(esito.slotLiberi).toContain(slotLibero.slotId);
  });

  it('settimanaTipoDefinitiva rifiuta se la stagione non è ancora definitiva', async () => {
    const stagioneId = await creaStagioneTest(pool);
    stagioniCreate.push(stagioneId);

    await expect(settimanaTipoDefinitiva(stagioneId)).rejects.toThrow();
  });
});
