...
import { listaOsservazioni, presentaOsservazione } from './osservazioni.ts';
...
descrivi('osservazioni.ts', () => {
  ...
  afterAll(async () => {
    ...
    // Ripulitura: osservazioni, domande e correlate, abilitazioni, associazioni, persone
  ...
  it('presentaOsservazione crea una nuova osservazione e listaOsservazioni la ritorna', async () => {
    ...
    await pool.query(
      `INSERT INTO discipline_sportive (codice, denominazione) VALUES ($1, 'Disciplina Test Osservazioni')`,
      [disciplinaCodice],
    );
    ...
    const associazione = await creaAssociazione({
      denominazione: `ASD Osservazioni Test ${suffisso}`,
      codiceFiscalePartitaIva: `PIVA-8-${suffisso}`,
      stagioneId,
      ...
      email: 'asd-osservazioni-test@example.com',
    ...
    // Cambia lo stato della domanda a 'esclusa' per poter presentare osservazioni
    await pool.query(
      `UPDATE domande SET stato = 'esclusa', motivazione_esclusione = 'Domanda non idonea' WHERE id = $1`,
      [domanda.id],
    );

    // Presenta un'osservazione
    const testoOsservazione = 'Ricorso contro l\'esclusione';
    const osservazioneCreata = await presentaOsservazione(domanda.id, testoOsservazione);

    expect(osservazioneCreata.id).toBeTruthy();
    expect(osservazioneCreata.domandaId).toBe(domanda.id);
    expect(osservazioneCreata.testo).toBe(testoOsservazione);
    expect(osservazioneCreata.presentataIl).toBeTruthy();
    expect(osservazioneCreata.stato).toBe('in_esame');
    expect(osservazioneCreata.decisioneMotivazione).toBeNull();
    expect(osservazioneCreata.decisaIl).toBeNull();

    // Verifica che listaOsservazioni la trovi
    const osservazioni = await listaOsservazioni(domanda.id);
    expect(Array.isArray(osservazioni)).toBe(true);
    expect(osservazioni.length).toBeGreaterThan(0);

    const osservazione = osservazioni.find((o) => o.id === osservazioneCreata.id);
    expect(osservazione).toBeTruthy();
    expect(osservazione!.testo).toBe(testoOsservazione);
    expect(osservazione!.stato).toBe('in_esame');
  });
});
