...
8. **Gestione stagionale** (B.32–B.36, Fase 15): ~~indisponibilità sopravvenute (B.33) + variazioni ordinarie tra associazioni (B.32: liberazione/recupero/scambio_temporaneo/utilizzo_occasionale)~~ ✅ **Fatto** — migration `000013`/`000014` (tabella `variazioni_ordinarie` + indice unico su destinazione), repository `indisponibilita.ts`/`variazioni.ts`, route `POST /backoffice/slot/:id/indisponibilita`, `GET /pubblico/associazioni/:id/indisponibilita`, `POST /pubblico/variazioni{,/:id/accetta,/:id/annulla}`, `GET /backoffice/stagioni/:id/variazioni` (sola lettura backoffice — nessuna coda di validazione attiva, istruzione esplicita del committente). Lock avviso per-occorrenza (`pg_advisory_xact_lock`) + indice unico parziale sulla destinazione contro race concorrenti; verifica esplicita di `indisponibile_permanente`/`indisponibilita_sopravvenute` e dello stato stagione (`STATI_STAGIONE_CON_DEFINITIVA`) prima di accettare una variazione. Design: `docs/superpowers/plans/2026-08-05-variazioni-ordinarie-indisponibilita.md`. ~~Rilevazione utilizzi (B.34) + escalation giustificazione→diffida→decadenza (B.35, soglie 🔧)~~ ✅ **Fatto** — migration `000015`/`000016` (ciclo di giustificazione su `utilizzi_effettivi` + parametro `termine_giustificazione_giorni` + indice unico su occorrenza), repository `utilizziEffettivi.ts`/`provvedimenti.ts`, route `POST /backoffice/assegnazioni/:id/utilizzi`, `GET /backoffice/assegnazioni/:id/utilizzi`, `PUT /backoffice/utilizzi/:id/{accogli,rigetta}-giustificazione`, `GET /backoffice/associazioni/:id/mancati-utilizzi`, `POST /backoffice/assegnazioni/:id/provvedimenti`, `GET /backoffice/assegnazioni/:id/provvedimenti`, `POST /pubblico/utilizzi/:id/giustificazione`, `GET /pubblico/associazioni/:id/utilizzi`. Decadenza (`assegnazioni.stato='decaduta'`) coerente col blocco precedente (`trovaProprietarioOccorrenza` la vede libera) e congela la convenzione collegata (`confermaConvenzione`/`listaConvenzioniPerStagione` verificano lo stato dell'assegnazione). `registraUtilizzo` verifica esplicitamente titolarità mobile per-occorrenza (variazioni ordinarie) e indisponibilità sopravvenute prima di imputare un mancato utilizzo — un'associazione non più titolare o un'occorrenza coperta da indisponibilità non contano mai ai fini della decadenza. Design: `docs/superpowers/specs/2026-08-09-monitoraggio-decadenza-design.md`, piano: `docs/superpowers/plans/2026-08-09-monitoraggio-decadenza.md`. Residuo B.36: solo storico interrogabile, nessun aggancio al calcolo CSD — rimandato alla Fase 7 (taratura CSD). **Fase 15 chiusa (B.32-36 completa lato backend)**.
