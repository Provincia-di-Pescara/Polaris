...
5. **Flusso pubblico** (🔶 in corso, blocchi 1/4 e 2/4 fatti): ~~accreditamento associazione + richiesta delega (art. 3–4), approvazione deleghe lato operatore~~ ✅ **Fatto**: `POST /pubblico/associazioni` (+ upload documenti su volume named, verifica PDF sui byte reali), `POST /pubblico/deleghe` (sub-delega gerarchica auto-approvata, catena tracciata via `creata_da_abilitazione_id`, cascata di revoca), `PUT /backoffice/deleghe/:id/{approva,respingi,revoca}`. Design: `docs/superpowers/specs/2026-07-30-accreditamento-delega-design.md`. ~~Domanda con fabbisogni/preferenze/blocchi/giornate gara (B.5–B.6), ammissibilità (B.7), pubblicazione esiti (B.10), osservazioni (B.11)~~ ✅ **Fatto**: `POST /pubblico/domande`, `GET /pubblico/{associazioni/:id/domande,domande/:id}`, `PUT /backoffice/domande/:id/{ammetti,escludi}`, `GET /backoffice/domande`(+`:id`), `GET /pubblico/stagioni/:id/domande/esiti`, `POST /pubblico/domande/:id/osservazioni`, `PUT /backoffice/osservazioni/:id/{accogli,respingi}`. Design: `docs/superpowers/specs/2026-08-01-domanda-osservazioni-design.md`. Riesame (B.11) tracciato su colonna `domande.riesame_stato` separata da `domande.stato` — il motore Go continua a leggere `stato='ammessa'` invariato. Restano: blocchi 3-4/4 (preferenze/concertazione lato pubblico, calendario).
