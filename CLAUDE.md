...
- **Middleware autorizzazione per ruolo** (`src/auth/middleware.ts`): `richiedeRuolo(...ruoli)` variadico, verifica audience JWT, applica a tutte le route CRUD backoffice (admin+operatore con poche eccezioni, solo admin per creazione stagioni).
...
- Testato end-to-end contro Postgres reale: tutti gli scenari CRUD (create, read, update, duplicato, non trovato, sovrapposizione slot, filtro impianto), ruoli autorizzati/negati, audit log tracciato. Test suite da 89 a 164 test, nessuna regressione.
