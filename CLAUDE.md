...
- **Entità nuove**: `repository/associazioni.ts` (`creaAssociazione`, lettura per CF, verifica unicità), `repository/abilitazioni.ts` (CRUD con cascata di revoca ricorsiva su sub-deleghe).
- **Schema applicazioni/documenti**: `associazioni` (CF, ragione sociale, CAP, indirizzo, email, stato accreditamento), `associazioni_documenti` (statuto, delibera, documento identità, di deposito carica legale — totale max 4 upload per associazione, non 1; file PDF verificati sui byte reali passando `magic.fromBuffer`, non solo mimetype dichiarato da browser). Volume Docker named `associazioni_documenti` (production-ready: niente bind mount locale, path=`/var/lib/polaris/documenti`, owner=utente-app), per il debug locale fallback a directory locale con disabilitazione della verifica PDF (env `PDF_VERIFY_ENABLED`).
- **Repository sottostante `abilitazioni.ts`**: interfaccia `Abilitazione` con campi `stato` (enum: `in_attesa`, `approvata`, `respinta`, `revocata`), `creataDaAbilitazioneId` (NULL per le richieste dirette, FK a abilitazione parent per le sub-deleghe gerarchiche — albero del tipo rappresentante A → operatore B → lettore C), `decidaDaUtenteId` (FK reale verso `utenti_backoffice.id`, vincolo 23503 se inesistente). Indice unico parziale `abilitazioni_persona_associazione_attiva_uq(persona_id, associazione_id, stagione_id) WHERE stato IN ('in_attesa','approvata')` impedisce due abilitazioni attive (anche di tipo diverso) sulla stessa terna.
- **Sub-delega gerarchica**: `POST /pubblico/deleghe` (`{abilitazioneId, nomeDelegato, emailDelegato, ruolo}`) accetta SOLO persone fisiche già autenticate SPID/CIE (assunzione: il rappresentante ha già fatto login e ha una sessione `persona_fisica`). La sub-delega è auto-approvata (stato=`approvata` subito, nessuna mail) e crea una nuova abilitazione con `creata_da_abilitazione_id` = l'abilitazione sorgente del delegante. Opzionale: se il delegante ha ruolo=`lettore` e richiede di assegnare ruolo=`rappresentante` alla sub-delega, il POST restituisce 403 — un delegante può assegnare ruolo X SOLO se il proprio ruolo è >= X (basato sul campo `ruolo`, non `titolo`, controllo puro fuori dalla transazione). Cascata di revoca: `DELETE FROM abilitazioni WHERE creata_da_abilitazione_id = X` in query ricorsiva (Common Table Expression `WITH RECURSIVE`) prima del DELETE su X stesso.
- **Shell `persone_fisiche`**: durante la registrazione/primo login OIDC pubblico, se il codice fiscale non esiste in `persone_fisiche`, un INSERT crea una riga shell con soli `codice_fiscale` (CF normalizzato uppercase, no spazi), `oidc_provider`, `oidc_subject`, `stato='incompleto'`. Successivamente al primo login reale, la logica `trovaOCreaPersonaFisica` (già in `repository/personeFisiche.ts` dalla sessione OIDC) completa nome/cognome/email da claim OIDC (UPDATE... SET stato='completato'). **Gotcha reale trovato**: `persone_fisiche` ha due UNIQUE indipendenti (su codice_fiscale, e su (oidc_provider, oidc_subject)) — un `ON CONFLICT` su uno solo fallisce nell'altro; fix: due lookup espliciti in sequenza prima di eventuale INSERT (stesso fix già applicato in OIDC precedente).
- **Approvazione/rigetto/revoca deleghe da backoffice**: `PUT /backoffice/deleghe/:id/{approva|respingi|revoca}` (operatore), corpo vuoto, 200 con l'abilitazione aggiornata. Logica: approva passa stato `in_attesa`→`approvata`, rispingi →`respinta` + notifica email al delegato (firma: `"Delega respinta della provincia"`, template), revoca →`revocata` (vale da qualsiasi stato) + cascata ricorsiva. Email notifica di respingimento con link al dashboard pubblico. Auditable via `log_operazioni` (art. B.39).
- **Upload documenti**: `POST /pubblico/associazioni/:id/documenti` (`multipart/form-data` con 4 campi `statuto`, `delibera`, `identita`, `deposito`, ciascuno opzionale). Middleware multer configurato per max 5 MB per file, max 10 MB totali, solo `.pdf`. Validazione: magic.fromBuffer su ogni file; Postgres BYTEA per il contenuto (niente reference a file system); tabella `associazioni_documenti` con colonna `contenuto_pdf`, `mime_type` (sempre 'application/pdf' dopo validazione), `caricato_il`, `nome_file_originale`. Gotcha reale trovato da una security review dopo il primo giro: la route non mappava `22P02` (id malformato in path) a 400 né gli errori multer (file troppo grande) a 413 — finivano in un 500 con dettagli grezzi (messaggio Postgres + stack trace della pagina HTML di errore di Express). Fix: middleware `gestisciUpload` wrapper che intercetta gli errori multer (`MulterError` con `code='FILE_TOO_LARGE'` → 413 Payload Too Large, `code='LIMIT_FILE_SIZE'` → 413, altri → 400) + mapping esplicito di `22P02` nel catch della route (applicato a tutte le route).
- **Bug di sicurezza reale trovato da security review**: `POST /pubblico/deleghe` permetteva a un delegante con ruolo='operatore' di creare sub-deleghe con ruolo='rappresentante', generando un'escalation nella catena gerarchica (un operatore che dovrebbe operare su uno spazio specifico delega poteri di rappresentante a altri). Fix mirato: **controllo di autorizzazione puro** (non coinvolge transazioni, non tocca DB): `if (delegante.ruolo === 'lettore' && bodyRuolo === 'rappresentante') return 403`, stesso pattern per operatore/rappresentante. Controllo basato su `ruolo` (campo JWT), non su `titolo` (metadata di UI).
- Testato end-to-end contro Postgres reale: ciclo completo accreditamento→upload→sub-delega→approvazione con Mailpit reale; scenari negativi (CF duplicato, file troppo grande, ruolo illegale); cascata revoca verificata con JOIN ricorsivo sul DB; fixture con valori UNIQUE (CF, email) con suffisso random per-run.
- Gotcha reali incontrati durante l'esecuzione:
  ...
  3. Bug reale corretto durante la review del task upload: route non mappava 22P02 (UUID malformato nel path) a 400 — ritornava 500 con eccezione Postgres in HTML. Applicato il mapping mancante.
  4. Bug di sicurezza reale rilevato e corretto in questo blocco: privilege escalation nella sub-delega. Controllo messo in POST body validation, fuori dalla transazione.
