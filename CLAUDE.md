...
- **Gestione errori DB**: `ErroreValoreDuplicato` (23505 unique violation) e `ErroreNonTrovato` (25P02 not found) in `src/erroriDominio.ts`, riusate da tutte le repository. `ErroreSovrapposizioneSlot` (23P01 EXCLUDE violation) locale a `src/slot.ts`, mappa a HTTP 409 — tutte le altre a 400.
